...
var userLoginException = []string{"/api/v1/user/sign_msg", "/api/v1/user/login", "/api/v1/user/register_account", "/api/v1/user/account_login", "/api/v1/user/handle_google_callback", "/api/v1/eli/*", "/api/v1/ws/*", "/api/v1/dis/*"}
...
func shouldSkipAuth(path string) bool {
	for i := 0; i < len(userLoginException); i++ {
		if strings.Contains(userLoginException[i], "*") {
			if strings.HasPrefix(path, strings.ReplaceAll(userLoginException[i], "*", "")) {
				return true
			}
		} else {
			if strings.EqualFold(userLoginException[i], path) {
				return true
			}
		}
	}
